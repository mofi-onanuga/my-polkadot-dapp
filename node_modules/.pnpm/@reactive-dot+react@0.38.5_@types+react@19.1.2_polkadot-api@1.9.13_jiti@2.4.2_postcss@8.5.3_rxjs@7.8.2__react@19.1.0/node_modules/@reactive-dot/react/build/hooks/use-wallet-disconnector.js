 ...
 */
export function useWalletDisconnector(wallets) {
    const hookWallets = wallets;
    const config = useConfig();
    return useAsyncAction(useAtomCallback(useCallback(async (get, _, wallets) => {
        const walletsToDisconnect = wallets ?? hookWallets ?? (await get(walletsAtom(config)));
        await disconnectWallet(walletsToDisconnect);
        return true;
    }, [config, hookWallets])));
}
//# sourceMappingURL=use-wallet-disconnector.js.map