...
export function useBlock(tag = "finalized", options) {
    ...
    return usePausableAtomValue(tag === "finalized"
 ...
 * @internal
 */
export const finalizedBlockAtom = atomFamilyWithErrorCatcher((withErrorCatcher, config, chainId) => atomWithObservableAndPromise((get) => from(get(clientAtom(config, chainId))).pipe(switchMap((client) => getBlock(client, { tag: "finalized" }))), withErrorCatcher));
/**
 * @internal
 */
export const bestBlockAtom = atomFamilyWithErrorCatcher((withErrorCatcher, config, chainId) => atomWithObservableAndPromise((get) => from(get(clientAtom(config, chainId))).pipe(switchMap((client) => getBlock(client, { tag: "best" }))), withErrorCatcher));
//# sourceMappingURL=use-block.js.map