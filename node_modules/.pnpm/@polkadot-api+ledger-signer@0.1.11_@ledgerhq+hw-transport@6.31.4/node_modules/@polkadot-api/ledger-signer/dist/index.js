...
const encodePath = (path1, path2) => {
  ...
  return utils.mergeUint8(
    START_PATH,
    substrateBindings.u32.enc(HARDENED + path1),
    MID_PATH,
    substrateBindings.u32.enc(HARDENED + path2)
  );
};
...
class LedgerSigner {
  ...
  async appInfo() {
    ...
    const appName = substrateBindings.Binary.fromBytes(res.slice(2, 2 + res[1])).asText();
    const appVersion = substrateBindings.Binary.fromBytes(
   ...
   */
  async deviceId() {
    return __privateGet(this, _deviceId) == null ? __privateSet(this, _deviceId, substrateBindings.u32.dec((await __privateMethod(this, _LedgerSigner_instances, getPublicKey_fn).call(this, 0, 0)).slice(0, 4))) : __privateGet(this, _deviceId);
  }
  /**
  ...
  async getPolkadotSigner(networkInfo, path1, path2 = 0) {
    ...
    const signTx = async (callData, signedExtensions, metadata) => {
      const merkleizer = merkleizeMetadata.merkleizeMetadata(metadata, networkInfo);
      ...
      v15.extrinsic.signedExtensions.map(({ identifier }) => {
        if (identifier === METADATA_IDENTIFIER) {
          ...
          additionalSigned.push(utils.mergeUint8(Uint8Array.from([1]), digest));
          return;
        }
      ...
      return signersCommon.createV4Tx(v15, publicKey, signature, extra, callData);
    };
    return {
      publicKey,
      signTx,
      signBytes: signersCommon.getSignBytes(
        async (x) => (
...
getPublicKey_fn = async function(path1, path2, seeAddressInDevice, ss58Prefix) {
  ...
  const bufToSend = Buffer.from(
    ...
    utils.mergeUint8(
      ...
      Uint8Array.from(substrateBindings.u16.enc(ss58Prefix ?? DEFAULT_SS58))
    )
  );
...
sign_fn = async function(path1, path2, payload, shortMetadata) {
  ...
  const chunks = [];
  chunks.push(Buffer.from(utils.mergeUint8(path, substrateBindings.u16.enc(payload.length))));
  const combinedPayload = shortMetadata == null ? payload : utils.mergeUint8(payload, shortMetadata);
  let offset = 0;
