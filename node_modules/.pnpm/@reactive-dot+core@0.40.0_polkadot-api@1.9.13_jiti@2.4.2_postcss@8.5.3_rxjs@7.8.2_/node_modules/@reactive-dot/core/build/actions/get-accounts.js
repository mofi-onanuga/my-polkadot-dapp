...
export function getAccounts(wallets, chainSpec) {
    return combineLatest([toObservable(wallets), toObservable(chainSpec)]).pipe(switchMap(([wallets, chainSpec]) => {
        if (wallets.length === 0) {
            return of([]);
        }
        ...
        return combineLatest(wallets.map((wallet) => wallet.accounts$.pipe(map((accounts) => accounts
            .map((account) => {
            const polkadotSigner = (() => {
                ...
                const nativeTokenInfo = nativeTokenInfoFromChainSpecData(chainSpec);
                return account.polkadotSigner({
            ...
            .filter((account) => account !== undefined))))).pipe(map((accounts) => accounts.flat()), map(chainSpec === undefined
            ? (accounts) => accounts
